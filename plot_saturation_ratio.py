...
def plot_saturation_ratio_contours(model, X, scaler_X, scaler_y):
    """Generates and returns saturation ratio contour plots."""
    if model is None:
    ...
    for i in range(5):
        for j in range(5):
            ...
            # Extract Saturation Ratio (SR) from predictions
            saturation_ratio = predictions[:, 1].reshape(grid_x.shape)

            cont_fill = ax.contourf(grid_x, grid_y, saturation_ratio, levels=5, cmap='viridis')
            cont_line = ax.contour(grid_x, grid_y, saturation_ratio, levels=5, colors='black', linewidths=0.5)
    ...
    fig.colorbar(cont_fill, cax=cbar_ax, label='Saturation Ratio')
